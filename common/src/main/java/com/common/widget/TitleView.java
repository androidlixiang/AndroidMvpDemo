...
public class TitleView extends LinearLayout {
    ...
    //默认中间的字体大小
    public static final int TEXTSISE = 17;
    ...
    public void initData(AttributeSet attrs) {
        ...
        mLeftIV.setImageResource(R.mipmap.back_black);
        ...
        //标题颜色
        mTitleName.setTextColor(ContextCompat.getColor(getContext(), R.color.c_333333));
        mTitleName.setSingleLine();
        mTitleName.setGravity(Gravity.CENTER);
        mTitleName.setEllipsize(TextUtils.TruncateAt.END);
//        mTitleName.getPaint().setFakeBoldText(true);
        ...
        mBottomView.setBackgroundResource(R.color.c_2b000000);
        //状态栏高度
        ...
        addView(ll, new LayoutParams(LayoutParams.MATCH_PARENT, DensityUtils.dp2px(NAVIGATIONHEIGHT)));
        addView(mBottomView, new LayoutParams(LayoutParams.MATCH_PARENT, DensityUtils.dp2px(0.5f)));
    }
    ...
    public void initAttrs(AttributeSet attrs) {
        if (attrs != null) {
            ...
            if (mLayoutResource != 0) {
                ll.removeAllViews();
                ViewGroup.LayoutParams layoutParams = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, DensityUtils.dp2px(NAVIGATIONHEIGHT));
            ...
            boolean showBack = typedArray.getBoolean(R.styleable.MyTitleView_showBack, true);
            if (!showBack) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (addmBottomView) {
            layoutParams.height = height + DensityUtils.dp2px(NAVIGATIONHEIGHT + 1);
        } else {
            layoutParams.height = height + DensityUtils.dp2px(NAVIGATIONHEIGHT);
    ...
    public abstract static class OnTitlerClickListener {
        protected void onBackClick(TitleView TitlerView, ImageView IVBack) {
            if (TitlerView.getContext() instanceof Activity)
                ((Activity) TitlerView.getContext()).finish();
        }

        protected void onTitleClick(TitleView TitlerView, String text) {
        }

        protected abstract void onRightClick(TitleView TitlerView, ImageView IVRight);
    }
